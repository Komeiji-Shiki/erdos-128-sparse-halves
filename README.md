...
programming, drafting and a subsequent audit. No human mathematical author
has yet checked the complete argument. This is not a peer-reviewed or
journal-accepted paper; research priority has not been established.
The computational checks do not replace the written proof or author review.
...
The PDF and downloadable ZIP preserve the pre-upload research snapshot.
References in them to an archive without a public URL describe the time at
which that snapshot was prepared; the computational files are now available here.
...
`manifest.json` records SHA-256 hashes of the initial repository payload.
The ZIP contains its own manifest for the earlier review snapshot. Running
...
当前候选上界为 0.02543n²，原题尚未解决，完整证明仍待人类作者核验。
