...
\title{Sparse-set extension and degree-sensitive perturbations\newline
for halves of triangle-free graphs}
\author{komeiji shiki}
\hypersetup{pdfauthor={komeiji shiki}}
\date{Revised manuscript for author review, 5 September 2026}
\begin{document}
\maketitle
\begin{abstract}
We give a computer-assisted argument that every triangle-free graph on $n$
vertices has $\lfloor n/2\rfloor$ vertices spanning at most
$(2543/100000)n^2=0.02543n^2$ edges. The argument makes degree-sensitive use
of the neighbourhood perturbation in Sarid's 2026 preprint. A
reciprocal-degree averaging inequality
reduces the improvement to a continuous inequality in three real variables;
a rational subdivision certificate verifies the entire parameter domain.
A robust sparse-set extension lemma, adapted from Razborov's
independent-set argument, supplies an additional candidate half.
We obtain another improvement by anchoring neighbourhoods across a
maximum cut and bounding their curvature through degree moments.
The remaining ingredients are the max-cut bounds of Balogh, Clemen and
Lidick\'y and Sarid's rational certificate for a four-cycle density inequality.
We recheck the latter using a separate implementation that exhausts all
labelled graphs on six vertices. The target $n^2/50$ of the sparse half
conjecture is not reached.
\end{abstract}

\noindent\textbf{Keywords:} triangle-free graphs, sparse halves, maximum cuts,
computer-assisted proofs.

\medskip
\noindent\textbf{Review status.}
The written argument has received a further AI-assisted mathematical audit,
and two separately implemented rational interval checkers agree.
Submission details and human verification of the complete proof are pending.
The AI-use disclosure and the scope of the checks appear at the end.
...
\]
These conditions ensure all hypotheses used to replace $Z$ by $Z_0$
and all positive-denominator comparisons. A lower bound for $W(Z_0)$ is
\[
 W_-=
...
\section*{Computational materials and AI-use disclosure}
The review archive contains the manuscript source, certificates, both
interval checkers, the four-cycle checker, and a SHA-256 manifest. File
paths refer to its root; a public URL has not yet been assigned.
...
where used. No human mathematical author has yet verified the complete
proof. The computational checks do not establish priority, replace author
responsibility, or constitute an end-to-end formalization. Author
details and human verification must be completed before submission.
...
\url{https://github.com/aimir/erdos-128-sparse-halves}.
